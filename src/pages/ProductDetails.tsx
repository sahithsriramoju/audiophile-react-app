        ...
        <div>
           <div className="flex flex-col md:flex-row lg:mt-40">
            <div className="flex flex-col items-start text-black-1 my-16 md:mr-8" key={productDetails?.id}>
                <img src={productDetails?.imageUrl} className="rounded-lg mb-8"></img>
            </div>
            <div className="mt-auto md:my-16 md:ml-8 flex flex-col items-start  md:justify-center">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    {true? <button className="hover:bg-light-brown bg-amber-700 p-4 cursor-pointer text-white text-xs font-bold uppercase"
                     onClick={()=>dispatch(incrementQuantity({
                        quantity:1, productId:productDetails?.id
                     }))}>
                    {(cart?.cartItems?.find(x=>x.productId === productDetails?.id)?.quantity ?? 0) == 0 ? "Add to Cart" : "Go to Bag" }</button> : <button disabled className="bg-amber-700 p-4 text-white">Out of Stock</button>}
                    </div>
                </div>
            </div>
            </div>
